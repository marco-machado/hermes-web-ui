/** Vendored subset of @hermes/shared JsonRpcGatewayClient (wire-compatible). */
...
export class JsonRpcGatewayClient {
  ...
  private readonly options: Required<GatewayClientOptions>
  ...
  async connect(wsUrl: string): Promise<void> {
    if (typeof wsUrl !== 'string' || !/^wss?:\/\//.test(wsUrl)) {
      throw new Error(`gateway connect() requires a ws:// or wss:// URL string, got ${String(wsUrl)}`)
    }
  ...
  request<T>(
    ...
    const id = `${this.options.requestIdPrefix}${++this.nextId}`
...
export function buildWsUrl(opts: {
  ...
  const path = opts.path ?? '/api/ws'
  const qs = new URLSearchParams({ token: opts.token })
  return `${scheme}://${opts.host}:${opts.port}${path}?${qs}`
}
